...
def _make_mock_model(vectors: list[list[float]]) -> MagicMock:
    """Return a mock SentenceTransformer that encodes to the given vectors."""
    mock_model = MagicMock()
    mock_model.encode.return_value = [np.array(v) for v in vectors]
    return mock_model


class TestGenerateEmbeddings:
    def test_returns_384_dimensions(self):
        mock_model = _make_mock_model([[0.1] * 384, [0.2] * 384])
        with patch.object(emb_module, "get_embedder", return_value=mock_model):
            result = generate_embeddings(["drought in Brazil", "CO2 emissions"])
        assert len(result) == 2
    ...
    def test_single_text(self):
        mock_model = _make_mock_model([[0.3] * 384])
        with patch.object(emb_module, "get_embedder", return_value=mock_model):
    ...
    def test_embeddings_are_floats(self):
        mock_model = _make_mock_model([[0.4] * 384])
        with patch.object(emb_module, "get_embedder", return_value=mock_model):
...
class TestGenerateQueryEmbedding:
    def test_returns_single_384_vector(self):
        mock_model = _make_mock_model([[0.5] * 384])
        with patch.object(emb_module, "get_embedder", return_value=mock_model):
